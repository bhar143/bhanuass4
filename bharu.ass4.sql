...
Boarding_City,p1.Bus_Type,p2.Price from passenger p1,price p2 where Passenger_Name = 'Pallavi'
...
select  Distance,Passenger_name,100 * count(*) / sum(count(*)) over() from passenger group by Distance,Passenger_name;

--- 11) Display the distance,price in the three categories in table Price
 a) Expensive if the cost is more than 1000 .
 
 select *  from price where Price >= 1000 ;
 b)Average Cost if the cost is less than 1000 and greater than 500.
 
 select * from price where Price <= 1000 and Price<(select Avg(Price) from price where Price >= 500);
